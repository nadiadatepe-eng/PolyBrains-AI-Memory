...
def patch(monty_cls):
    ...
    def traced_vote(self):
        if self.lm_to_lm_vote_matrix is None:
            return orig_vote(self)
        # Run one round at a time so each round is attributable.
        total = self.vote_rounds
        for r in range(total):
            self._probe_round = r
            silent = sum(1 for lm in self.learning_modules
                         if lm.send_out_vote() is None)
            self.vote_rounds = 1
            try:
                orig_vote(self)
            finally:
                self.vote_rounds = total
            TRACE["rounds"].append(
...
def main():
    ...
    from polybrains.iterated_vote import IteratedVotingMonty

    register_resolvers()
    patch(IteratedVotingMonty)
    ...
    with initialize_config_dir(
        ...
        cfg = compose(
            ...
            overrides=[
                "experiment=p9_rounds3",
                "++experiment.config.seed=42",
                "++experiment.config.n_eval_epochs=1",
                "++experiment.config.logging.wandb_id=p9probe",
                "++experiment.config.logging.run_name=p9_deadprobe",
            ],
        )
